...
def get_config():
    ...
    C.model.attn_pdrop = 0.0
    C.model.embd_pdrop = 0.0
    C.model.resid_pdrop = 0.0
...
class AdditionDataset(Dataset):
    ...
    def __init__(self, config, flags, split):
        self.config = config
        self.flags = flags
        ...
        perm = perm[:self.flags.training_dataset_size+num_test]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--training_dataset_size', type=int, default=500)
    ...
    config = get_config()
    config.trainer.batch_size = flags.training_batch_size
    print(config)
    setup_logging(config)
    set_seed(flags.seed)

    # construct train and test datasets
    train_dataset = AdditionDataset(config.data, flags, split='train')
    test_dataset  = AdditionDataset(config.data, flags, split='test')
